...
export default async function VerifyPage({
  ...
  // Підтвердження пошти (і magic-link доводить володіння скринькою)
  if (!user.emailVerified) {
    ...
    await prisma.notification.create({
      data: {
        ...
        body: "Дякуємо! Тепер ви можете користуватись платформою.",
        link: "/welcome",
      },
    });
  }
  ...
  if (!refreshed) {
    return <ErrorPanel title="Помилка" hint="Спробуйте пізніше." />;
  }

  if (result.type === "magic") {
    redirect(
      refreshed.role === "member" && refreshed.subscriptionPlan === "free" ? "/welcome" : "/dashboard",
    );
  }

  redirect(refreshed.role === "member" && refreshed.subscriptionPlan === "free" ? "/welcome" : "/dashboard");
}
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4">
      <div className="max-w-md w-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm text-center">
        ...
        <div className="mt-6 flex gap-3 justify-center">
          ...
          <Link href="/register" className="text-sm font-medium text-zinc-500 hover:underline">
            Зареєструватись
          </Link>
        </div>
      </div>
